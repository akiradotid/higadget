  ...
  class Datatables
  {
    ...
    */
    private function get_filtering()
    {
      ...
      $sSearch = isset($search['value']) && !empty($search['value']) ? $this->ci->db->escape_like_str(trim($search['value'])) : null;
    ...
    */
    private function get_total_results($filtering = FALSE)
    {
      ...
      $subquery = $this->ci->db->select($this->select)->from($this->table)->get_compiled_select();
    ...
    */
    private function check_cType()
    {
      ...
      if (!empty($columns) && is_numeric($columns[0]['data'])) {
          return FALSE; // If it's numeric, return FALSE
      } else {
          return TRUE; // If it's not numeric or the array is empty, return TRUE
      }
