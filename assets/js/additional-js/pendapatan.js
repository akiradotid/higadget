        ...
        ajax: {
            ...
            data: function (params) {
                return {
                    ...
                    tipe: 'Pendapatan', // Add the search term to your AJAX request
                };
            },
...
function reload() {
    if ($.fn.DataTable.isDataTable('#table-finance-pendapatan')) {
      tsupp.destroy();
    }
    tsupp = $("#table-finance-pendapatan").DataTable({
        "processing": true,
        "language": { 
        ...
        "ajax": {
            "url": base_url+'pendapatan/jsonfnp',
            "type": "POST"
        },
        "columns": [
            { "data": "id_finance_trx"},
...
function getid(){
  $('#EditFinancePendapatanModal').on('show.bs.modal', function (e) {
      var button = $(e.relatedTarget);
      var id_sup = button.data('id');
      
      $.ajax({
          url: base_url + "pendapatan/edit/"+id_sup,
          dataType: "json",
...
function updatedata(){
  $("#update").on("click", function (){
      ...
      $.ajax({
          type: "POST",
          url: base_url + 'pendapatan/update-data',
          data: {
              eid: id,
...
function deletedata() {
  $('#table-finance-pendapatan').on('click', '#delete-btn', function (e) {
      e.preventDefault();
      var id = $(this).data('id');
      swal({
          ...
          if (willDelete) {
              $.ajax({
                  type: 'POST',
                  url: base_url + 'pendapatan/hapus/' + id,
                  dataType: 'json',
                  success: function (response) {
                      if (response.success) {
